...
namespace muse::rpc {
    ...
    port(_port)
    {
        ...
        struct timeval tm = {request_timeout.count() / 1000, (request_timeout.count() * 1000) % 1000000 };
    ...
    bool Transmitter::send(TransmitterEvent&& event) {
        //判断是否是多次发送同一个 event
        if (new_messages.count(event.get_message_id()) != 0) return false;
        if (event.get_callBack_state() && event.get_remote_state()){
            ...
            taskPtr->message_id = taskPtr->event.get_message_id();
            // 获得发送次数
            ...
            taskPtr->last_piece_size = (taskPtr->total_size % Protocol::defaultPieceLimitSize == 0)? Protocol::defaultPieceLimitSize:taskPtr->total_size % Protocol::defaultPieceLimitSize ; /* 数据部分 */
            {
                std::unique_lock<std::mutex> lock(new_messages_mtx);
                // 插入到任务重
                if (messages.count(taskPtr->event.get_message_id()) == 0)
                {
                    messages[taskPtr->event.get_message_id()] =  taskPtr;
                    return true;
                }
            }
        }
